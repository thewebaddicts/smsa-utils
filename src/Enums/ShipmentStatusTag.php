...
enum ShipmentStatusTag: string
{
    ...
    {
        return match ($this) {
            self::GENERAL_AWB_EXCEPTION => 'This is happening on the mobile app then putting a general exception for the awb on pickup or delivery',
            ...
            self::DELIVERY_EXCEPTION => 'Last Mile flow: problems while attempting or completing delivery to the consignee without scanning the awb.',
            self::DELIVERY_TRIP_EXCEPTION => 'Delivery trip: courier on-the-road issues during delivery execution.',
    ...
    public function screenshots(): string
    {
        $file = match ($this) {
            self::GENERAL_AWB_EXCEPTION => 'general-awb-exception.jpg',
            self::PICKUP_EXCEPTION => 'pickup-exception.jpg',
            self::PICKUP_TRIP_EXCEPTION => 'pickup-trip-exception.jpg',
            self::DELIVERY_EXCEPTION => 'delivery-exception.jpg',
            self::DELIVERY_TRIP_EXCEPTION => 'delivery-trip-exception.jpg',
            self::EXCEPTION => 'exception.png',
            self::WORKFLOW => 'workflow.png',
          
            // self::WORKFLOW_TRIP_EXCEPTION => 'Grouping for workflow logic statuses dropdown.',
           
            self::PROBLEMATIC_DEBRIEF_EXCEPTION => 'problematic-debrief-exception.png',
            self::HOLD_EXCEPTION => 'hold-exception.png',
            self::ATTEMPTED => 'attempted.png',
            self::REFUSED => 'refused.png',

            default => null,
        };

        return $file ? asset('images/shipment-status-tags/' . $file) : '';
    }
